...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] == UPLOAD_ERR_OK) {
        ...
        if (in_array($_FILES['profile_image']['type'], $allowed_types)) {
            ...
            if (move_uploaded_file($_FILES['profile_image']['tmp_name'], $upload_file)) {
                $profile_image = $upload_file; // Store the path to the uploaded image
            } else {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Insert into the database with profile image
...
<body class="bg-dark flex flex-col items-center justify-center h-screen">
    <div class="flex rounded-3xl p-6 flex-col justify-between text-center py-8 w-80 bg-light-10 slide-in" id="register-form">
        ...
        <form method="POST" action="register.php" enctype="multipart/form-data">
            <div class="flex flex-col items-start mb-4">
                ...
                <input class="bg-dark p-2 text-xs text-white flex h-8 w-full shadow-sm" type="text" name="username" id="username" placeholder="username" required>
            </div>
            <div class="flex flex-col items-start mb-4">
                <h1 class="text-white text-xs mb-2" for="email">Email</h1>
                <input placeholder="example@gmail.com" class="bg-dark p-2 text-xs text-white flex h-8 w-full shadow-sm" type="email" name="email" id="email" required>
            </div>
        ...
        <h1 class="text-white text-xs">Already have an account? <a class="font-bold text-secondary" href="login.php">Login here</a></h1>
    </div>
